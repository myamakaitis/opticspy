...
if __name__ == '__main__':
    ...
    pm1 = roc.PointSource(-1.1, 0, t_max, num = 31, color='#00FF00')
    p0  = roc.PointSource(-1.0, 0, t_max, num = 31, color = '#0000FF')
    p1  = roc.PointSource(-0.9, 0, t_max, num = 31, color = '#FF0000')
    ...
    Sensor = roc.Image(.13, .0008, intensity=.75)

    fig, ax = pyp.subplots(dpi = 200)
    ax.grid(True)

    ax.hlines([-3/2 * pmla,-pmla/2,pmla/2,3/2 * pmla],-1.5,2, color = 'k')
    ...
    cm1 = roc.CollimatedSource(.025, num = 25, theta =-.05, color = 'winter',zstart=-1)
    c0  = roc.CollimatedSource(.025, num = 25, theta =  0,  color = 'autumn',zstart=-1)
    c1  = roc.CollimatedSource(.025, num = 25, theta = .02, color = 'cool',zstart=-1)
